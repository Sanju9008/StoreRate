...
export default function SortableHeader({ label, columnKey, currentSort, currentOrder, onSort }) {
    ...
    const handleSort = () => {
        if (isActive) {
            ...
            onSort(columnKey, 'DESC'); // Default to DESC for new column
        }
    };
        ...
        >
            <div className="flex items-center gap-1 select-none">
                ...
                <span className="text-gray-400">
                    {isActive ? (
                        currentOrder === 'ASC' ? <ArrowUp className="w-4 h-4 text-blue-600" /> : <ArrowDown className="w-4 h-4 text-blue-600" />
                    ) : (
