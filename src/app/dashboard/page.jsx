...
export default function NormalUserDashboard() {
    ...
    const handleRate = async (storeId) => {
        ...
        // Optimistically find previous rating for rollback
        const storeToRate = stores.find(s => s.id === storeId);
        const prevRating = storeToRate ? storeToRate.userSubmittedRating || 0 : 0;
        ...
        try {
            const res = await fetch('/api/ratings', {
                ...
                body: JSON.stringify({ storeId, rating: newRating })
            });
            const data = await res.json();
            ...
            if (res.ok && data.success) {
                ...
                // Revert on error
                setUserRatings(prev => ({ ...prev, [storeId]: prevRating }));
        ...
        <ProtectedRoute allowedRoles={['NORMAL_USER']}>
            <div className="bg-[#F8FAFC] min-h-screen font-sans">
                ...
                <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    {/* Unified Top Toolbar */}
                    {/* Unified Top Toolbar */}
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pb-6 border-b border-slate-200/80 mb-8 mt-4">
                        {/* Left: Text Area */}
                        ...
                        <div className="flex-1 w-full sm:w-auto flex justify-center">
                            <div className="relative w-full max-w-md">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-10 pr-4 py-2.5 min-h-[44px] bg-white border border-[#CBD5E1] rounded-xl text-sm text-[#0F172A] placeholder-[#94A3B8] shadow-sm focus:border-blue-600 focus:ring-2 focus:ring-blue-600/10 outline-none transition-all"
                                />
                            </div>
                        </div>
                        ...
                        <div className="shrink-0 w-full sm:w-auto">
                            <select 
                                ...
                                className="bg-white border border-[#CBD5E1] rounded-xl px-4 py-2.5 min-h-[44px] text-sm text-[#0F172A] shadow-sm focus:border-blue-600 outline-none flex items-center gap-2 cursor-pointer w-full sm:w-auto"
                            >
                                    ...
                                    <div key={store.id} className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm hover:shadow-md hover:border-[#CBD5E1] transition-all duration-200 p-5 flex flex-col justify-between">
                                        ...
                                        <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-3.5 mt-4 flex items-center justify-between">
                                            ...
                                            <button
                                                ...
                                                className="bg-[#2563EB] hover:bg-[#1D4ED8] active:scale-95 text-white font-medium text-xs px-4 py-2 min-h-[44px] rounded-lg shadow-sm shadow-blue-500/20 transition-all flex items-center justify-center min-w-[70px] disabled:opacity-50 disabled:bg-slate-300 disabled:cursor-not-allowed"
                                            >
